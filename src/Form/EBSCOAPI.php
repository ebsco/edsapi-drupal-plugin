...
class EBSCOAPI
{
     ...
     * @global array
     */
    private static $mapped_sort_options = [
        '' => 'relevance',
        'relevance' => 'relevance',
        'subject' => 'date',
        'date' => 'date2',
    ...
    protected function request($action, $params = null, $attempts = 5)
    {
        ...
        $response = call_user_func_array([$this->connector(), "request{$action}"], [$params, $headers]);
    ...
    public function apiSearch($search,
        ...
        if (!empty($search['lookfor'])) {
            ...
            if (preg_match('/(.*) (AND|OR) (.*)/i', $term)) {
                ...
                $op = 'AND';
                $query_str = implode(',', [$op, $tag]);
                $query_str = implode(($tag ? ':' : ''), [$query_str, $term]);
